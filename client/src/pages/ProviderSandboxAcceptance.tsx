...
const evidenceStates = ["pending", "submitted", "verified_by_provider", "rejected"] as const;
type EvidenceState = typeof evidenceStates[number];
...
export default function ProviderSandboxAcceptance() {
  ...
  const [mtls, setMtls] = useState<EvidenceState>("pending");
  const [reconciliation, setReconciliation] = useState<EvidenceState>("pending");
  ...
  const save = () => {
    ...
    if (mtls === "verified_by_provider" && !sandboxBaseUrl.trim()) return toast.error("A provider-issued HTTPS sandbox URL is required for verified mTLS evidence.");
    if ((mtls === "verified_by_provider" || reconciliation === "verified_by_provider") && !providerReference.trim()) return toast.error("A provider-issued test reference is required for verified evidence.");
    if (mtls === "verified_by_provider" && reconciliation === "verified_by_provider" && !attestation.trim()) return toast.error("A bilateral attestation reference is required when both evidence tracks are verified.");
    submit.mutate({ providerName, sandboxBaseUrl: sandboxBaseUrl || undefined, providerReference: providerReference || undefined, mtlsEvidenceState: mtls, reconciliationEvidenceState: reconciliation, bilateralAttestationReference: attestation || undefined, evidenceNotes: notes || undefined });
  };
  ...
  return <div className="space-y-6"><div><div className="flex items-center gap-2 text-sm text-muted-foreground"><LockKeyhole className="h-4 w-4 text-amber-600" /> Settlement gate control</div><h1 className="mt-1 text-3xl font-bold tracking-tight">Provider/FSP Sandbox Acceptance</h1><p className="mt-1 max-w-3xl text-muted-foreground">Record bilateral sandbox evidence for review. This workspace cannot enable payment execution or self-certify a provider relationship.</p></div>
    ...
    <div className="grid gap-6 lg:grid-cols-[1.05fr_.95fr]"><Card><CardHeader><CardTitle>Submit acceptance evidence</CardTitle><CardDescription>Capture the sandbox endpoint, provider references, and bilateral acceptance references without storing private keys or tokens.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="rounded-md border border-sky-200 bg-sky-50 p-3 text-sm text-sky-950 dark:border-sky-900 dark:bg-sky-950/30 dark:text-sky-100"><p className="font-medium">Evidence requirements</p><ul className="mt-1 list-disc space-y-1 pl-4 text-xs"><li>Use an externally reachable HTTPS endpoint, never a local URL.</li><li>Verified evidence requires a provider-issued test reference.</li><li>When both evidence tracks are verified, provide an independent bilateral attestation reference.</li><li>Do not submit certificates, private keys, or bearer tokens.</li></ul></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Provider / FSP name"><Input value={providerName} onChange={e => setProviderName(e.target.value)} placeholder="Example FSP Sandbox" /></Field><Field label="Sandbox URL"><Input value={sandboxBaseUrl} onChange={e => setSandboxBaseUrl(e.target.value)} placeholder="https://sandbox.provider.example" /></Field><Field label="Provider test reference"><Input value={providerReference} onChange={e => setProviderReference(e.target.value)} placeholder="Provider-issued reference" /></Field><Field label="Bilateral attestation reference"><Input value={attestation} onChange={e => setAttestation(e.target.value)} placeholder="Acceptance report ID" /></Field><Field label="mTLS evidence"><Select value={mtls} onValueChange={value => setMtls(value as EvidenceState)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{evidenceStates.map(s => <SelectItem key={s} value={s}>{s.replaceAll("_", " ")}</SelectItem>)}</SelectContent></Select></Field><Field label="Reconciliation evidence"><Select value={reconciliation} onValueChange={value => setReconciliation(value as EvidenceState)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{evidenceStates.map(s => <SelectItem key={s} value={s}>{s.replaceAll("_", " ")}</SelectItem>)}</SelectContent></Select></Field></div><Field label="Evidence notes"><Textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Describe the provider-issued report schema, callback test, and independent reviewer." /></Field><Button onClick={save} disabled={submit.isPending}><Upload className="mr-2 h-4 w-4" />Save evidence record</Button></CardContent></Card>

...
import { AlertTriangle, ArrowRight, BriefcaseBusiness, CheckCircle2, CircleDollarSign, Search } from "lucide-react";
...
export default function ProviderDisputeManagement() {
  ...
  return <div className="space-y-6">
    ...
    <Dialog open={Boolean(selected)} onOpenChange={open => !open && setSelected(null)}><DialogContent><DialogHeader><DialogTitle>Review {selected?.referenceNumber}</DialogTitle><DialogDescription>This action does not resolve a dispute automatically. It takes you to the controlled record workflow, which applies access checks and validates the next permitted transition.</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={() => setSelected(null)}>Cancel</Button><Button onClick={() => selected && navigate(`/disputes/${selected.id}`)}>Open controlled workflow <ArrowRight className="ml-2 h-4 w-4" /></Button></DialogFooter></DialogContent></Dialog>
  </div>;
}

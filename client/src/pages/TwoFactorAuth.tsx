...
export default function TwoFactorAuth() {
  ...
  const disableMutation = trpc.totp.disable.useMutation({
    onSuccess: () => {
      utils.totp.status.invalidate();
      utils.totp.getBackupCodes.invalidate();
      setStep("status");
      setDisableCode("");
  ...
  function copyBackupCodes() {
    const codes = backupCodes.length ? backupCodes : (backupCodesData?.backupCodes ?? []);
    navigator.clipboard.writeText(codes.join("\n"));
    setCopiedCodes(true);
    setTimeout(() => setCopiedCodes(false), 2000);
  }

  const displayBackupCodes = backupCodes.length ? backupCodes : (backupCodesData?.backupCodes ?? []);
  const usedCount = backupCodesData?.usedBackupCodes?.length ?? 0;
                ...
                <div className="flex items-center gap-3 p-4 rounded-lg bg-green-50 border border-green-200">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-green-600">
                      ...
                      {totpStatus?.enabledAt && ` · Enabled ${new Date(totpStatus.enabledAt).toLocaleDateString()}`}
                    </div>
                  </div>
                </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                className="flex-1"
                disabled={verifyCode.length !== 6 || setupMutation.isPending}
              >
                {setupMutation.isPending ? <Loader2 size={14} className="mr-2 animate-spin" /> : null}
                Verify Code
              </Button>
            </div>
          </CardContent>
      ...
      {step === "setup-backup" && (
        <Card className="border-slate-200">
          ...
          <CardContent className="space-y-4">
            ...
            <Button onClick={() => setStep("setup-done")} className="w-full">I've saved my backup codes</Button>
          </CardContent>
        </Card>
      )}
        ...
        <Card className="border-red-200">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              >
                {disableMutation.isPending ? <Loader2 size={14} className="mr-2 animate-spin" /> : null}
                Disable 2FA
              </Button>
            </div>
          </CardContent>

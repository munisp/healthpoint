...
export default function TemporalOperations() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-4">
      <Metric title="Configuration" value={readiness.data?.configured ? "Ready" : "Blocked"} detail={readiness.data?.configured ? `${readiness.data.namespace} / ${readiness.data.taskQueue}` : readiness.data?.reason ?? "Loading configuration"} icon={<ShieldCheck className="h-5 w-5" />} positive={Boolean(readiness.data?.configured)} warning={readiness.data?.configured === false} />

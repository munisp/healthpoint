...
import { Activity, Plus, Search, RefreshCw, ChevronLeft, ChevronRight, AlertCircle } from 'lucide-react';
...
export default function AuditLog() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      setItems(data.items || data.audit || data.data || []);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-slate-500">View immutable audit trail of all platform actions.</p>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase">User Name</th>
              ...
              <tbody>
                {items.map((item, idx) => (
                  <tr key={item.id || idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.user_name ?? '—')}</td>
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.action ?? '—')}</td>
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.resource ?? '—')}</td>
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.ip_address ?? '—')}</td>
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.timestamp ?? '—')}</td>

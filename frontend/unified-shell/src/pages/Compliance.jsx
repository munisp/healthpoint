...
import { Shield, Plus, Search, RefreshCw, ChevronLeft, ChevronRight, AlertCircle } from 'lucide-react';
...
export default function Compliance() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const res = await authFetch(`${API_BASE}/api/v1/compliance/reports?${params}`);
      if (!res?.ok) throw new Error(`HTTP ${res?.status}`);
      const data = await res.json();
      setItems(data.items || data.reports || data.data || []);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-slate-500">Monitor NSA regulatory compliance, deadlines, and reporting requirements.</p>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase">Report Type</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase">Period</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase">Status</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase">Submitted At</th>
              ...
              <tbody>
                {items.map((item, idx) => (
                  <tr key={item.id || idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.report_type ?? '—')}</td>
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.period ?? '—')}</td>
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.status ?? '—')}</td>
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.submitted_at ?? '—')}</td>
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.score ?? '—')}</td>
                    <td className="px-4 py-3 text-right">
                      <button
                        onClick={() => navigate(`/compliance/reports/${item.id}`)}

...
import { Stethoscope, Plus, Search, RefreshCw, ChevronLeft, ChevronRight, AlertCircle } from 'lucide-react';
...
export default function ClaimsProcessing() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const res = await authFetch(`${API_BASE}/api/v1/claims?${params}`);
      if (!res?.ok) throw new Error(`HTTP ${res?.status}`);
      const data = await res.json();
      setItems(data.items || data.claims || data.data || []);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-slate-500">Process and adjudicate health insurance claims.</p>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase">Service Code</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase">Billed Amount</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase">Allowed Amount</th>
              ...
              <tbody>
                {items.map((item, idx) => (
                  <tr key={item.id || idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.service_code ?? '—')}</td>
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.billed_amount ?? '—')}</td>
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.allowed_amount ?? '—')}</td>

...
import { BarChart3, Plus, Search, RefreshCw, ChevronLeft, ChevronRight, AlertCircle } from 'lucide-react';
...
export default function Analytics() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const res = await authFetch(`${API_BASE}/api/v1/analytics/reports?${params}`);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-slate-500">View platform-wide analytics, dispute trends, and financial summaries.</p>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase">Report Name</th>
              ...
              <tbody>
                {items.map((item, idx) => (
                  <tr key={item.id || idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.report_name ?? '—')}</td>
                    <td className="px-4 py-3 text-sm text-slate-700">{String(item.type ?? '—')}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button
                        onClick={() => navigate(`/analytics/reports/${item.id}`)}

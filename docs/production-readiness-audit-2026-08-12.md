...
| External funds transfer / settlement rail | 45/100 | Versioned HMAC callbacks, trusted mTLS ingress, provider fingerprint allowlisting, immutable transfer lifecycle states, maker-checker approvals, independent signed provider-report reconciliation, exception evidence, immutable reversals, atomic ledger changes, and Playwright coverage exist. The Go sidecar still targets a simulator; no verified regulated FSP/bank integration or live settlement certification exists. |
...
| Overall production readiness for handling or initiating real funds | **42/100** | **Not approved for real funds.** The new controls materially improve intent, approval, reconciliation, and reversal integrity but do not establish an independently verified payment rail or guarantee against compromise. |
...
7. Complete daily balance proof, role-separation review for the production identity provider, immutable audit exports, operational alerting, and provider-specific reversal/correction acceptance criteria.

 ...
 */
/**
 * Normalize a service category: the deprecated 'AIR_AMBIANCE' typo alias is
 * accepted on read for wire-compat but is NEVER emitted downstream — all
 * rationale text and program-scope matching use the canonical 'AIR_AMBULANCE'.
 */
export function normalizeServiceCategory(category: ServiceCategory): ServiceCategory {
  return category === 'AIR_AMBIANCE' ? 'AIR_AMBULANCE' : category;
}
...
export function resolveJurisdiction(input: JurisdictionInput): JurisdictionResult {
  ...
  if (serviceCategory !== input.serviceCategory) {
    warnings.push(
      `serviceCategory '${input.serviceCategory}' is a deprecated typo alias and was normalized to '${serviceCategory}'.`,
    );
  }
  ...
  switch (program.scopeVsFederal) {
    case 'FULL':
      ...
      return {
        ...
        stateProgramId: program.stateCode,
        rationale:
          `Fully-insured plan in ${input.stateCode}: the registered state program covers the full federal surprise-billing scope, so the state process governs this ${serviceCategory} service.`,
        verificationStatus: program.verificationStatus,
        warnings,
      };
    case 'PARTIAL':
      ...
      return {
        ...
        stateProgramId: program.stateCode,
        rationale:
          `Fully-insured plan in ${input.stateCode}: the registered state program covers only a portion of federal surprise-billing protections (bifurcated process). The state regime governs items within state-law scope; the FEDERAL IDR process applies to out-of-scope items, including any ${serviceCategory} services not covered under state law. ` +
          FEDERAL_FLOOR,
        verificationStatus: program.verificationStatus,
        warnings: [
          ...warnings,

...
export interface ContractedRateRow {
   ...
   * The (b)(1) median is over CONTRACTED rates — one rate per contract:
   * rows sharing a contractId are collapsed to that contract's median rate
   * BEFORE the overall median is computed, so a single contract with many
   * provider rows cannot dominate the median population.
   */
  contractId?: string;
}
...
export function computeMedianContractedRate(
  ...
  // (b)(1): the median is over contracted rates — ONE rate per contract.
  // Rows sharing a contractId are collapsed to the contract's median rate
  // before entering the median population; rows without a contractId each
  // count individually (one implicit contract per row).
  ...
  rows.forEach((row, i) => {
    ...
    const key = row.contractId ?? `__row_${i}`;
    const bucket = byContract.get(key) ?? [];
    bucket.push(effectiveRateCents(row));
    byContract.set(key, bucket);
  });
  for (const rates of byContract.values()) {
    rates.sort((a, b) => a - b);
    eligible.push(median(rates));
  }

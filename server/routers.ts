...
export const appRouter = router({
  ...
  reports: router({
    ...
    exportCSV: protectedProcedure
      .input(z.object({ startDate: z.string().optional(), dateRangeLabel: z.string().default("Last 6 months"), executiveSummary: z.string().optional() }))
      .mutation(async ({ ctx, input }) => {
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database unavailable" });
        const { disputes: disputesSchema } = await import("../drizzle/schema");
        const allDisputes = await db.select().from(disputesSchema).where(eq(disputesSchema.createdBy, ctx.user.id)).orderBy(desc(disputesSchema.createdAt));
        const startMs = input.startDate ? new Date(input.startDate).getTime() : 0;
        const filtered = startMs > 0 ? allDisputes.filter(d => (d.createdAt?.getTime() ?? 0) >= startMs) : allDisputes;
        const closed = filtered.filter(d => d.status === "closed");
        ...
        const avgDetermination = closed.length ? Math.round(closed.reduce((s, d) => s + Number(d.determinationAmount ?? 0), 0) / closed.length) : 0;
        const avgDaysToClose = closed.length ? Math.round(closed.reduce((s, d) => { const ms = (d.updatedAt?.getTime() ?? Date.now()) - (d.createdAt?.getTime() ?? Date.now()); return s + ms / 86400000; }, 0) / closed.length) : 0;
        const totalBilled = Math.round(filtered.reduce((s, d) => s + Number(d.billedAmount ?? 0), 0));
        const totalQPA = Math.round(filtered.reduce((s, d) => s + Number(d.qpaAmount ?? 0), 0));
        const byServiceType = Object.entries(filtered.reduce<Record<string,number>>((acc, d) => { const k = d.serviceType ?? "unknown"; acc[k] = (acc[k] ?? 0) + 1; return acc; }, {})).map(([type, count]) => ({ type, count })).sort((a, b) => b.count - a.count);
        const MONTHS = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
        const monthMap: Record<string, { open_negotiation: number; idr_active: number; closed: number; ineligible: number }> = {};
        ...
        const financialByServiceType = Object.entries(finMap).map(([serviceType, v]) => ({ serviceType, avgBilled: Math.round(v.billed.reduce((a,b)=>a+b,0)/v.billed.length), avgQPA: Math.round(v.qpa.reduce((a,b)=>a+b,0)/v.qpa.length), avgDetermination: Math.round(v.det.filter(x=>x>0).reduce((a,b)=>a+b,0)/(v.det.filter(x=>x>0).length||1)) }));
        const outcomeMonthMap: Record<string, { won: number; lost: number; pending: number }> = {};
        for (const d of closed) { const dt = d.closedAt ?? d.updatedAt ?? new Date(); const key = `${MONTHS[dt.getMonth()]} ${dt.getFullYear()}`; if (!outcomeMonthMap[key]) outcomeMonthMap[key] = { won: 0, lost: 0, pending: 0 }; if (Number(d.determinationAmount ?? 0) >= Number(d.qpaAmount ?? 0)) outcomeMonthMap[key].won++; else outcomeMonthMap[key].lost++; }
        const outcomeByMonth = Object.entries(outcomeMonthMap).map(([month, v]) => ({ month, ...v }));
        const csv = generateReportsCSV({ dateRangeLabel: input.dateRangeLabel, generatedAt: new Date().toISOString(), executiveSummary: input.executiveSummary, metrics: { totalDisputes: filtered.length, closed: closed.length, winRate, avgDetermination, avgDaysToClose, totalBilled, totalQPA, inProgress: filtered.filter(d => d.status !== "closed" && d.status !== "ineligible").length, ineligible: filtered.filter(d => d.status === "ineligible").length }, byMonth, financialByServiceType, outcomeByMonth, avgDaysByStep: [], byServiceType, disputes: filtered });
        const filename = `HealthPoint-IDR-Report-${input.dateRangeLabel.replace(/\s+/g, "-")}-${new Date().toISOString().slice(0,10)}.csv`;
        return { csv, filename, rowCount: filtered.length };
      }),

    exportPDF: protectedProcedure
      .input(z.object({ startDate: z.string().optional(), dateRangeLabel: z.string().default("Last 6 months"), executiveSummary: z.string().optional(), executiveSummaryGeneratedAt: z.string().optional() }))
      .mutation(async ({ ctx, input }) => {
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database unavailable" });
        const { disputes: disputesSchema } = await import("../drizzle/schema");
        ...
        for (const d of closed) { const dt = d.closedAt ?? d.updatedAt ?? new Date(); const key = `${MONTHS[dt.getMonth()]} ${dt.getFullYear()}`; if (!outcomeMonthMap[key]) outcomeMonthMap[key] = { won: 0, lost: 0, pending: 0 }; if (Number(d.determinationAmount ?? 0) >= Number(d.qpaAmount ?? 0)) outcomeMonthMap[key].won++; else outcomeMonthMap[key].lost++; }
        const outcomeByMonth = Object.entries(outcomeMonthMap).map(([month, v]) => ({ month, ...v }));
        const IDR_STEPS = ["STEP_1_OPEN_NEGOTIATION","STEP_2_IDR_INITIATION","STEP_3_ENTITY_SELECTION","STEP_4_ELIGIBILITY_REVIEW","STEP_5_OFFER_SUBMISSION","STEP_6_DETERMINATION","STEP_7_PAYMENT","STEP_8_APPEAL","STEP_9_COMPLIANCE","STEP_10_CLOSURE"];
        const stepDayMap: Record<string, number[]> = {};
        for (const d of closed) { const step = d.currentStep ?? ""; if (!stepDayMap[step]) stepDayMap[step] = []; const ms = (d.updatedAt?.getTime() ?? Date.now()) - (d.createdAt?.getTime() ?? Date.now()); stepDayMap[step].push(ms / 86400000); }
        const avgDaysByStep = IDR_STEPS.map(step => ({ step: step.replace("STEP_", "Step ").replace(/_/g, " "), avgDays: stepDayMap[step]?.length ? Math.round(stepDayMap[step].reduce((a,b)=>a+b,0)/stepDayMap[step].length) : 0 }));
        const pdfBuffer = await generateReportsPDF({ dateRangeLabel: input.dateRangeLabel, generatedAt: input.executiveSummaryGeneratedAt ?? new Date().toISOString(), executiveSummary: input.executiveSummary, metrics: { totalDisputes: filtered.length, closed: closed.length, winRate, avgDetermination, avgDaysToClose, totalBilled, totalQPA, inProgress: filtered.filter(d => d.status !== "closed" && d.status !== "ineligible").length, ineligible: filtered.filter(d => d.status === "ineligible").length }, byMonth, financialByServiceType, outcomeByMonth, avgDaysByStep, byServiceType, disputes: filtered });
        const filename = `HealthPoint-IDR-Report-${input.dateRangeLabel.replace(/\s+/g, "-")}-${new Date().toISOString().slice(0,10)}.pdf`;
        return { base64: pdfBuffer.toString("base64"), filename, contentType: "application/pdf", pageCount: Math.ceil(filtered.length / 40) + 5 };
      }),

    generateExecutiveSummary: protectedProcedure
      .input(z.object({
        startDate: z.string().optional(),
        dateRangeLabel: z.string().default("Last 6 months"),
      }))
      .mutation(async ({ ctx, input }) => {
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database unavailable" });
        ...
        const totalBilled = Math.round(filtered.reduce((s, d) => s + Number(d.billedAmount ?? 0), 0));
        const totalQPA = Math.round(filtered.reduce((s, d) => s + Number(d.qpaAmount ?? 0), 0));
        const byServiceType = Object.entries(filtered.reduce<Record<string,number>>((acc, d) => { const k = d.serviceType ?? "unknown"; acc[k] = (acc[k] ?? 0) + 1; return acc; }, {})).sort((a, b) => b[1] - a[1]).slice(0, 5);
        const inProgress = filtered.filter(d => d.status !== "closed" && d.status !== "ineligible").length;
        const ineligible = filtered.filter(d => d.status === "ineligible").length;
        const prompt = `You are an expert healthcare payment dispute analyst. Generate a professional executive summary for a HealthPoint IDR platform report.

Report Period: ${input.dateRangeLabel}
Generated: ${new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}

Key Metrics:
- Total Disputes Filed: ${filtered.length}
- Closed / Resolved: ${closed.length}
- In Progress: ${inProgress}
- Ineligible: ${ineligible}
- Win Rate (initiating party): ${winRate}%
- Average Determination Amount: $${avgDetermination.toLocaleString()}
- Average Days to Close: ${avgDaysToClose} days
- Total Billed Amount: $${totalBilled.toLocaleString()}
- Total QPA Benchmark: $${totalQPA.toLocaleString()}
- Top Service Types: ${byServiceType.map(([t, c]) => `${t.replace(/_/g, " ")} (${c})`).join(", ")}

Write a 3-paragraph executive summary:
1. Overall performance overview (dispute volume, resolution rate, key trends)
2. Financial analysis (billed vs QPA vs determinations, win rate interpretation, cost implications)
3. Strategic recommendations and compliance observations based on the NSA/IDR regulatory framework

Use professional, concise language suitable for healthcare executives and compliance officers. Do not use bullet points — write in full paragraphs only.`;
        const response = await invokeLLM({
          messages: [
            { role: "system", content: "You are a senior healthcare payment dispute analyst specializing in NSA/IDR compliance. Write executive summaries that are precise, data-driven, and actionable. Always write in complete paragraphs, never bullet points." },
            { role: "user", content: prompt },
          ],
        });
        const rawContent = response.choices?.[0]?.message?.content;
        const summary: string = typeof rawContent === "string" ? rawContent : Array.isArray(rawContent) ? (rawContent as Array<{ type: string; text?: string }>).filter(c => c.type === "text").map(c => c.text ?? "").join("") || "Executive summary generation failed. Please try again." : "Executive summary generation failed. Please try again.";
        return {
          summary,
          generatedAt: new Date().toISOString(),
          metrics: { totalDisputes: filtered.length, closed: closed.length, winRate, avgDetermination, avgDaysToClose, totalBilled, totalQPA, inProgress, ineligible },
        };
      }),
  }),
  // ─── Audit Trail ─────────────────────────────────────────────────────────────

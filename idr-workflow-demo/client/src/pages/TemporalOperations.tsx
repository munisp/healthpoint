...
import { AlertTriangle, CheckCircle2, Database, Loader2, RefreshCw, RotateCcw, Search, ShieldCheck, WifiOff, Workflow } from "lucide-react";
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <p className="mt-1 max-w-3xl text-muted-foreground">Monitor strict-TLS workflow connectivity and inspect PostgreSQL-backed evidence for explicitly requested workflow dispatches.</p>
      ...
      <div className="flex gap-2"><Button variant="outline" onClick={refresh}><RefreshCw className="mr-2 h-4 w-4" />Refresh</Button></div>
    </div>
    ...
    <div className="grid gap-4 md:grid-cols-4">
      ...
      <Metric title="Real dispatch" value={readiness.data?.dispatchEnabled ? "Enabled" : "Disabled"} detail="Explicitly requested, governed workflow dispatch only" icon={<Workflow className="h-5 w-5" />} warning={!readiness.data?.dispatchEnabled} />
      <Metric title="Dispatch evidence" value={history.data?.length ?? 0} detail="Durable PostgreSQL audit records" icon={<Database className="h-5 w-5" />} positive={(history.data?.length ?? 0) > 0} />

...
describe("settlement lifecycle controls", () => {
  ...
  it("accepts only provider-report statuses that can be independently reconciled", () => {
    expect(providerSettlementReportSchema.safeParse({
      ...
      providerTransferId: "provider-transfer-1", status: "settled", amountCents: "10000", currency: "USD", reportedAt: new Date().toISOString(),
    }).success).toBe(true);
    expect(providerSettlementReportSchema.safeParse({
      provider: "mojaloop", reportId: "report-0001", transferId: "11111111-1111-4111-8111-111111111111",
      providerTransferId: "provider-transfer-1", status: "requested", amountCents: "10000", currency: "USD", reportedAt: new Date().toISOString(),
    }).success).toBe(false);
  });
});

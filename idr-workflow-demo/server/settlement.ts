...
import { parseCentsString, recordPaymentInTransaction, LedgerIntegrityError } from "./ledger";
    ...
    if (input.status === "settled") {
      ...
      if (wasPaymentDetermination) {
        ...
        await tx.insert(disputeEvents).values({
          ...
          description: `Authenticated ${input.provider} settlement callback reconciled for ${input.amountCents.toString()} cents.`,

...
import {
  ...
  deleteDisputeDraft,
  calculateQPA,
  getIDREntityCaseload,
  listAllIDREntityCaseloads,
  saveCMSDraft,
  getCMSDraftByDispute,
  listCMSDraftsByUser,
  updateCMSDraftStatus,
  getDisputesByMonth,
  listAllCMSDrafts,
  createEMRConnection,
  listEMRConnections,
  getEMRConnection,
  updateEMRConnectionStatus,
  deactivateEMRConnection,
  deleteEMRConnection,
  listEMRSyncLogs,
  createEMRSyncLog,
  createDisputeTemplate,
  listDisputeTemplates,
  getDisputeTemplateById,
...
import { ManualCmsHandoffAdapter } from "./services/cms-adapter";
import { PostgresCmsSubmissionStore } from "./services/cms-outbox";
import { storageReadVerified } from "./storage";
import {
  getGovernedOutcomePrediction,
  invokeGovernedGeorgetownRuntime,
  persistGovernedOutcomePrediction,
  requireGovernedOutcomeContext,
} from "./services/governed-outcome";
import {
  advanceWorkflow,
  IDR_WORKFLOW_STEPS,
  getWorkflowProgress,
...
import {
  initializeDisputeLedger,
  recordBilledAmount,
  recordAllowedAmount,
  recordDetermination,
  recordPayment,
  getDisputeBalances,
  getDisputeLedgerHistory,
  getDisputeFinancialSummary,
  parseCentsString,
} from "./ledger";
import { dispatchOutboxBatch } from "./outbox";
import {
  createSettlementTransfer,
  decideSettlementTransfer,
  getSettlementTransfer,
  listSettlementTransfers,
  markSettlementTransferSubmitted,
} from "./settlement-lifecycle";
import {
  activateTigerBeetleFinalityMapping,
  approveTigerBeetleFinalitySubmissionAuthorization,
  cancelTigerBeetleFinalitySubmissionAuthorization,
  createTigerBeetleFinalityMappingDraft,
  executeTigerBeetleFinalitySubmissionAuthorization,
  getTigerBeetleFinalityMappingActivationReadiness,
  listTigerBeetleFinalityIntentsForDispute,
  listTigerBeetleFinalityMappingDrafts,
  listTigerBeetleFinalitySubmissionAuthorizations,
  requestTigerBeetleFinalitySubmissionAuthorization,
} from "./tigerbeetle-finality";
import {
  listSettlementBalanceProofs,
  listSettlementExceptionReviews,
  reviewSettlementException,
} from "./settlement-proof";
import { configureDailyBalanceProofSchedule } from "./settlement-proof";
import { listSchedulerJobs } from "./scheduler-operations";
import { parse as parseCookie } from "cookie";
import {
  search,
  generateLakehouseExport,
...
import {
  ...
  changelogEntries,
  emrConnections,
  providerSandboxAcceptances,
} from "../drizzle/schema";
import { dispatchNotification } from "./notifications";
import {
  describeTemporalFailure,
  getDisputeTemporalWorkflow,
  getTemporalClient,
  getTemporalConfiguration,
  isTemporalDispatchEnabled,
  listTemporalWorkflows,
  startDisputeTemporalWorkflow,
  summarizeTemporalConnectionFailures,
  type TemporalRecoveryDetails,
} from "./temporal";
...
const advanceStepSchema = z.object({
  ...
  determinationWinner: z
    .enum(["initiating_party", "responding_party"])
    .optional(),
});
...
const ALLOWED_DISPUTE_DOCUMENT_MIME_TYPES = new Set([
  "application/pdf",
  "image/jpeg",
  "image/png",
  "image/tiff",
...
const MAX_DISPUTE_DOCUMENT_BYTES = 25 * 1024 * 1024;

function assertSafeDisputeDocumentInput(input: {
  disputeId: string;
  fileName: string;
  fileType: string;
  fileSize: number;
  ...
  const mimeType = input.fileType.toLowerCase().split(";", 1)[0];
  if (!ALLOWED_DISPUTE_DOCUMENT_MIME_TYPES.has(mimeType)) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "Document MIME type is not allowed for dispute evidence",
    });
  }
  if (!Number.isSafeInteger(input.fileSize) || input.fileSize < 1 || input.fileSize > MAX_DISPUTE_DOCUMENT_BYTES) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "Document size must be between 1 byte and 25 MiB",
    });
  }
  const normalizedKey = input.storageKey.trim().replace(/^\/+/, "");
  const requiredPrefix = `disputes/${input.disputeId}/`;
  if (
    normalizedKey !== input.storageKey ||
    !normalizedKey.startsWith(requiredPrefix) ||
    normalizedKey.includes("..")
  ) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "Document storage key must be a normalized private key for this dispute",
    });
  }
  if (input.fileName.includes("/") || input.fileName.includes("\\") || input.fileName.includes("\u0000")) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "Document filename must not contain path separators or control characters",
    });
  }
}
...
export const appRouter = router({
    ...
    latencyProbe: publicProcedure.query(async () => {
      ...
      return {
        ...
        api: {
          latency: apiLatency,
          status:
            apiLatency > 1000 ? ("degraded" as const) : ("healthy" as const),
        },
      ...
      .query(async ({ input }) => {
        ...
        for (let i = input.days - 1; i >= 0; i--) {
          ...
          result.push({
            ...
            total:
              Number(openedRow?.count ?? 0) + Number(closedRow?.count ?? 0),
    ...
    outcomeAnalytics: protectedProcedure.query(async () => {
      ...
      const rows = (await db.execute(
        ...
        rows: {
          ...
          total: string;
          wins: string;
          avgDeterminationAmount: string;
          avgBilledAmount: string;
        }[];
      };
      ...
      .query(async ({ input }) => {
        ...
        return {
          rows: (result.rows ?? []).map(r => ({
            ...
            winRate:
              Number(r.total) > 0
                ? Math.round((Number(r.wins) / Number(r.total)) * 100)
                : 0,
  ...
  disputes: router({
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        initializeDisputeLedger(dispute.id).catch(e =>
          console.warn(
            "[Ledger] Failed to initialize ledger for dispute",
            dispute.id,
            e
          )
        );
    ...
    advance: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          validateWorkflowTransition(current.currentStep, newStep, {
            ...current,
            ...additionalData,
          });
        } catch (error) {
        ...
        const dispute = await withDisputeLock(disputeId, 10_000, () =>
          advanceDisputeStep(
            disputeId,
            newStep,
            getStatusForStep(newStep),
            ctx.user.id,
            ...
            description,
            {
              ...
              ineligibilityReason:
                additionalData.ineligibilityReason ?? undefined,
              determinationBasis:
                additionalData.determinationBasis ?? undefined,
              determinationWinner:
                additionalData.determinationWinner ?? undefined,
            }
          )
        );
    ...
    selectArbitrator: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        return withDisputeLock(input.disputeId, 10_000, () =>
          advanceDisputeStep(
            input.disputeId,
            "STEP_07_IDR_ENTITY_SELECTED",
            ...
            {
              idrEntityId: input.idrEntityId,
              idrEntityName: input.idrEntityName,
            }
          )
        );
      }),
    ...
    getTimeline: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const currentStepIndex = IDR_STEP.indexOf(
          dispute.currentStep as (typeof IDR_STEP)[number]
        );
      ...
      .query(async ({ ctx, input }) => {
        ...
        const rows = items.map(d =>
          [
            ...
            d.initiatingPartyOffer != null
              ? Number(d.initiatingPartyOffer).toFixed(2)
              : "",
            d.respondingPartyOffer != null
              ? Number(d.respondingPartyOffer).toFixed(2)
              : "",
            d.determinationAmount != null
              ? Number(d.determinationAmount).toFixed(2)
              : "",
            d.patientState,
            d.facilityState,
            d.openNegotiationDeadline
              ? new Date(d.openNegotiationDeadline).toLocaleDateString()
              : "",
            d.offerSubmissionDeadline
              ? new Date(d.offerSubmissionDeadline).toLocaleDateString()
              : "",
            d.paymentDeadline
              ? new Date(d.paymentDeadline).toLocaleDateString()
              : "",
    ...
    findDuplicates: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (input.claimNumber)
          conditions.push(
            ilike(disputesTable.referenceNumber, `%${input.claimNumber}%`)
          );
        if (input.payerName)
          conditions.push(
            ilike(disputesTable.respondingPartyName, `%${input.payerName}%`)
          );
    ...
    rejectOffer: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const { disputeEvents: disputeEventsTable } = await import(
          "../drizzle/schema"
        );
        ...
        await db.insert(disputeEventsTable).values({
          ...
          description: input.reason
            ? `Offer rejected: ${input.reason}`
            : "Offer rejected by initiating party",
        ...
        await createNotification({
          ...
          message: input.reason
            ? `Offer was rejected: ${input.reason}`
            : "The offer has been rejected and the dispute has been closed.",
          dueDate: null,
        });
        return { success: true };
      }),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (db) {
          ...
          await db.insert(disputeEventsTable).values({
            ...
            performedByName: ctx.user.name ?? "Unknown",
            metadata: {
              sourceDisputeId: input.disputeId,
              sourceRef: original.referenceNumber,
            },
          });
        }
        return {
          ...
          referenceNumber: newDispute.referenceNumber,
        };
      }),
    ...
    merge: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if (input.primaryDisputeId === input.secondaryDisputeId) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Cannot merge a dispute with itself",
          });
        }
        const db = await getDb();
        ...
        if (!primary || !secondary)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "One or both disputes not found",
          });
        ...
        await db
          ...
          .set({
            ...
            notes:
              `[Merged into ${primary.referenceNumber}] ${secondary.notes ?? ""}`.trim(),
            updatedAt: new Date(),
          })
        ...
        await db.insert(disputeEventsTable).values({
          ...
          performedByName: ctx.user.name ?? "Unknown",
          metadata: {
            mergedDisputeId: input.secondaryDisputeId,
            mergedRef: secondary.referenceNumber,
    ...
    caseload: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        if (!result)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "IDR entity not found",
          });
        return result;
      }),

    allCaseloads: protectedProcedure.query(async () => {
      await seedIDREntities();
  ...
  documents: router({
    upload: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        assertSafeDisputeDocumentInput(input);
        return addDocument({
          disputeId: input.disputeId,
          uploadedBy: ctx.user.id,
    ...
    listVersions: protectedProcedure
      .input(z.object({ documentId: z.string().min(1), disputeId: z.string().min(1) }))
      .query(async ({ input, ctx }) => {
        await assertDisputeAccess(ctx.user.id, ctx.user.role, input.disputeId, "read");
        const db = await getDb();
        if (!db) return [];
    ...
    uploadVersion: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const { documentVersions, disputeDocuments } = await import("../drizzle/schema");
        const { and, eq } = await import("drizzle-orm");
        const [document] = await db
          .select({ id: disputeDocuments.id })
          .from(disputeDocuments)
          .where(
            and(
              eq(disputeDocuments.id, input.documentId),
              eq(disputeDocuments.disputeId, input.disputeId)
            )
          )
          .limit(1);
        if (!document) {
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Document does not belong to the supplied dispute",
          });
        }
        ...
        const [{ maxVer }] = await db
          .select({ maxVer: max(documentVersions.versionNumber) })
          .from(documentVersions)
          .where(eq(documentVersions.documentId, input.documentId));
  ...
  // --- CMS human portal handoff -------------------------------------------------
  cms: router({
    prepareManualHandoff: protectedProcedure
      .input(
        z.object({
          disputeId: z.string().min(1),
          idempotencyKey: z.string().min(16).max(191),
          pilotAuthorizationId: z.string().min(1),
          schemaVersion: z.string().min(1).max(32),
          openNegotiationEndedAt: z.string().datetime(),
          submissionDeadline: z.string().datetime(),
          claimNumbers: z.array(z.string().min(1)).min(1).max(100),
          serviceCodes: z.array(z.string().min(1)).min(1).max(100),
          serviceDates: z.array(z.string().min(1)).min(1).max(100),
          serviceLocations: z.array(z.string().min(1)).min(1).max(100),
          serviceType: z.string().min(1).max(128),
          initiatingParty: z.record(z.string(), z.unknown()),
          respondingParty: z.record(z.string(), z.unknown()),
          eligibilityAttestation: z.literal(true),
          preferredIdrEntity: z.string().min(1).max(256).optional(),
          totalPaymentAmountCents: z.number().int().positive(),
          currency: z.literal("USD"),
          documentIds: z.array(z.string().min(1)).min(1).max(25),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (new Set(input.documentIds).size !== input.documentIds.length) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "CMS handoff document IDs must be unique",
          });
        }
        const db = await getDb();
        if (!db) {
          throw new TRPCError({
            ...
            message: "PostgreSQL is required for a durable CMS manual handoff",
          });
        }
        const { disputeDocuments } = await import("../drizzle/schema");
        ...
        const persistedDocuments = await db
          ...
          .where(
            and(
              eq(disputeDocuments.disputeId, input.disputeId),
              inArray(disputeDocuments.id, input.documentIds)
            )
          );
        if (persistedDocuments.length !== input.documentIds.length) {
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "One or more CMS handoff documents do not belong to this dispute",
          });
        }
        const byId = new Map(persistedDocuments.map(document => [document.id, document]));
        const documents = await Promise.all(
          input.documentIds.map(async documentId => {
            const document = byId.get(documentId);
            if (!document?.s3Key || !document.mimeType || !document.fileName) {
              throw new TRPCError({
                code: "PRECONDITION_FAILED",
                message: "CMS handoff requires server-stored document key, filename, and MIME type",
              });
            }
            const stored = await storageReadVerified(document.s3Key);
            const storedMimeType = stored.contentType?.split(";", 1)[0]?.toLowerCase();
            if (storedMimeType && storedMimeType !== document.mimeType.toLowerCase()) {
              throw new TRPCError({
                code: "PRECONDITION_FAILED",
                message: "Stored document MIME type does not match the dispute record",
              });
            }
            return {
        ...
        const handoff = await new ManualCmsHandoffAdapter(
          new PostgresCmsSubmissionStore()
        ).prepareHandoff({
          ...input,
          initiatedBy: ctx.user.id,
          handoffOperatorId: ctx.user.id,
          documents,
        });
        return {
          submissionId: handoff.submissionId,
          disputeId: handoff.disputeId,
          pilotAuthorizationId: handoff.pilotAuthorizationId,
          handoffOperatorId: handoff.handoffOperatorId,
          payloadHash: handoff.payloadHash,
          status: handoff.status,
          createdAt: handoff.createdAt,
          manualPortalActionRequired: true as const,
        };
      }),

    listMyPendingManualHandoffs: protectedProcedure
      .input(z.object({ limit: z.number().int().min(1).max(100).default(25) }).optional())
      .query(async ({ ctx, input }) => {
        const handoffs = await new PostgresCmsSubmissionStore().listPendingManualHandoffs(
          ctx.user.id,
          input?.limit ?? 25
        );
        for (const handoff of handoffs) {
          await assertDisputeAccess(ctx.user.id, ctx.user.role, handoff.disputeId, "write");
        }
        return handoffs.map(({ payload: _payload, ...handoff }) => handoff);
      }),

    recordManualPortalReceipt: protectedProcedure
      .input(
        z.object({
          submissionId: z.string().min(1),
          cmsReference: z.string().min(1).max(128),
          receivedAt: z.string().datetime(),
          receiptSha256: z.string().regex(/^[a-fA-F0-9]{64}$/),
          outcome: z.enum(["accepted", "rejected"]),
          rejectionReason: z.string().min(1).max(4000).optional(),
        })
      )
      .mutation(async ({ ctx, input }) => {
        const store = new PostgresCmsSubmissionStore();
        const handoff = await store.findBySubmissionId(input.submissionId);
        if (!handoff) {
          throw new TRPCError({ code: "NOT_FOUND", message: "CMS handoff record was not found" });
        }
        await assertDisputeAccess(ctx.user.id, ctx.user.role, handoff.disputeId, "write");
        if (handoff.handoffOperatorId !== ctx.user.id) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Only the authorized handoff operator may record the CMS portal receipt",
          });
        }
        const receipt = await new ManualCmsHandoffAdapter(store).recordHumanPortalReceipt({
          ...input,
          recordedBy: ctx.user.id,
        });
        return { ...receipt, manualPortalActionRequired: false as const };
      }),
  }),
  ...
  ai: router({
    ...
    agentInfo: publicProcedure.query(async () => {
      try {
        const res = await fetch(`${AI_SERVICE_URL}/agent-info`, {
          signal: AbortSignal.timeout(5000),
        });
          ...
          if (dispute) {
            disputeContext = {
              ...
              serviceDate: dispute.serviceDate
                ? new Date(dispute.serviceDate).toLocaleDateString()
                : undefined,
    ...
    listCMSDrafts: protectedProcedure
      .input(
        z.object({ adminAll: z.boolean().optional().default(false) }).optional()
      )
      .query(async ({ ctx, input }) => {
    ...
    // Optional decision support for a dispute-bound human CMS portal handoff.
    // This output never submits to CMS and never replaces authorized human review.
    validateCMSSubmission: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        try {
          ...
          // A degraded validation response must not imply that deterministic,
          // regulatory, or document checks have passed. Production callers
          // receive an explicit service error and must complete human review.
          if (ENV.isProduction) {
            throw new TRPCError({
              code: "SERVICE_UNAVAILABLE",
              message:
                "CMS decision-support validation is unavailable; complete the required human portal review before submission.",
              cause: error,
            });
          }
          return {
            ...
            issues: [
              {
                ...
                message:
                  "The AI validation service is temporarily unavailable. Manual review is recommended before submitting to CMS.",
                remediation:
                  "Proceed with manual review or retry validation when the AI service is restored.",
              },
            ],
            layer_results: {
              schema: false,
            ...
            remediation_plan: [
              "Manually verify all required fields are complete",
              "Confirm all required documents are attached",
              "Review the submission narrative for completeness",
            ],
            summary:
              "AI validation service unavailable. Submission requires manual review.",
          };
        }
      }),

    // Optional decision-support edits for an authorized dispute-bound draft.
        ...
        try {
          ...
          if (ENV.isProduction) {
            throw new TRPCError({
              code: "SERVICE_UNAVAILABLE",
              message:
                "CMS decision-support auto-fix is unavailable; apply corrections through authorized human review.",
              cause: error,
            });
          }
          return {
            ...
            unfixableIssues: input.issues
              ...
              .map(i => ({
                code: i.code,
                field: i.field ?? null,
                reason: "AI auto-fix service unavailable",
              })),
            fixCount: 0,
            unfixableCount: input.issues.filter(i => i.severity === "blocking")
              .length,
            summary:
              "AI auto-fix service is temporarily unavailable. Please apply corrections manually.",
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          ...
          return {
            ...
            fhirResources: [],
            summary:
              "EMR data extraction service is temporarily unavailable. Please enter dispute fields manually.",
    ...
    searchPatients: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        try {
          const result = await aiPost<{
            patients: { id: string; name: string; dob: string; mrn: string }[];
          }>("/search-patients", {
            connectionId: input.connectionId,
            emrSystem: input.emrSystem,
            query: input.query,
          ...
          try {
            ...
            const conn = await db
              .select()
              .from(emrConnections)
              .where(eq(emrConnections.id, input.connectionId))
              .limit(1);
            ...
            const resp = await fetch(
              `${baseUrl}/Patient?${params.toString()}`,
              {
            ...
            return (bundle.entry ?? []).map(e => ({
              ...
              name: e.resource.name?.[0]
                ? `${e.resource.name[0].family ?? ""}, ${(e.resource.name[0].given ?? []).join(" ")}`.trim()
                : "Unknown",
              dob: e.resource.birthDate ?? "",
              mrn:
                e.resource.identifier?.find(i => i.value)?.value ??
                e.resource.id,
            }));
          } catch {
            return [];
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          ...
          const fallback = {
            ...
            resourcesFound: ["Patient", "Claim"],
            mappingValidation: [],
            aiAnalysis: "Fallback test",
          };
          await createEMRSyncLog({
            id: crypto.randomUUID(),
            connectionId: input.connectionId,
            ...
            status: "success",
            fieldsExtracted: 2,
            fieldConfidence: { overall: 0.85 },
            fhirResourcesAccessed: ["Patient", "Claim"],
            warnings: ["AI service unavailable — offline test"],
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          ...
          const message =
            error instanceof Error
              ? error.message
              : "EMR connection test failed";
          const failedResult = {
            success: false,
            message: `FHIR connection test failed for ${input.baseUrl}: ${message}`,
            resourcesFound: [],
            mappingValidation: Object.keys(input.fieldMappings).map(field => ({
              field,
              status: "not_verified",
            })),
            aiAnalysis:
              "No production readiness conclusion was generated because the configured FHIR endpoint could not be verified.",
            confidence: 0,
          };
          if (input.connectionId) {
            await createEMRSyncLog({
              ...
              fhirResourcesAccessed: [],
              warnings: ["FHIR endpoint verification failed"],
              summary: failedResult.message,
              durationMs: Date.now() - startMs,
              triggeredBy: ctx.user.id,
            }).catch(() => {
              /* non-blocking */
  ...
  stateLaws: router({
    list: publicProcedure
      .input(
        z.object({
          state: z.string().optional(),
          hasProtection: z.boolean().optional(),
        })
      )
      .query(async ({ input }) => {
        ...
        const STATE_LAWS = [
          {
            ...
            name: "California",
            hasProtection: true,
            lawName: "SB 1021 / AB 72",
            effectiveDate: "2017-07-01",
            scope: "Emergency + Non-emergency out-of-network",
            idrProcess: "Independent Dispute Resolution",
            maxPenalty: "$25,000 per violation",
            notes:
              "Strongest state protections; applies to fully-insured plans",
          },
          {
            state: "NY",
            name: "New York",
            hasProtection: true,
            lawName: "NY Surprise Bill Law",
            effectiveDate: "2015-03-31",
            scope: "Emergency + Non-emergency out-of-network",
            idrProcess: "Independent Dispute Resolution",
            maxPenalty: "$10,000 per violation",
            notes: "First state surprise billing law; model for federal NSA",
          },
          {
            state: "TX",
            name: "Texas",
            ...
            idrProcess: "Mediation for amounts > $500",
            maxPenalty: "$5,000 per violation",
            notes: "Mediation-based resolution",
          },
          {
            state: "FL",
            name: "Florida",
            hasProtection: true,
            lawName: "FS 627.64194",
            effectiveDate: "2016-07-01",
            scope: "Emergency services",
            idrProcess: "Negotiation required",
            maxPenalty: "License action",
            notes: "Applies to state-regulated plans only",
          },
          {
            state: "IL",
            name: "Illinois",
            ...
            scope: "Emergency + Non-emergency",
            idrProcess: "IDR",
            maxPenalty: "$10,000 per violation",
            notes: "Mirrors federal NSA provisions",
          },
          {
            state: "WA",
            name: "Washington",
            ...
            idrProcess: "IDR",
            maxPenalty: "$5,000 per violation",
            notes: "Broad consumer protections",
          },
          {
            state: "CO",
            name: "Colorado",
            ...
            idrProcess: "IDR",
            maxPenalty: "$5,000 per violation",
            notes: "Applies to state-regulated plans",
          },
          {
            state: "NJ",
            name: "New Jersey",
            ...
            scope: "Emergency + Non-emergency",
            idrProcess: "Arbitration",
            maxPenalty: "$10,000 per violation",
            notes: "Arbitration-based resolution",
          },
          {
            state: "AZ",
            name: "Arizona",
            hasProtection: false,
            lawName: "No state law",
            effectiveDate: null,
            scope: "Federal NSA only",
            idrProcess: "Federal NSA IDR",
            maxPenalty: null,
            notes: "Relies on federal NSA protections",
          },
          {
            state: "GA",
            name: "Georgia",
          ...
          },
          {
            ...
            scope: "Emergency services",
            idrProcess: "Negotiation",
            maxPenalty: "$1,000 per violation",
            notes: "Limited scope",
          },
          {
            state: "PA",
            ...
            idrProcess: "IDR",
            maxPenalty: "$5,000 per violation",
            notes: "Comprehensive protections",
          },
          {
            state: "MI",
            name: "Michigan",
          ...
          },
          {
            ...
            idrProcess: "IDR",
            maxPenalty: "$5,000 per violation",
            notes: "Comprehensive state protections",
          },
          {
            state: "MA",
            name: "Massachusetts",
        ...
        if (stateFilter)
          results = results.filter(l => l.state === stateFilter.toUpperCase());
        if (input.hasProtection !== undefined)
          results = results.filter(
            l => l.hasProtection === input.hasProtection
          );
        return {
          laws: results,
          total: results.length,
          withProtection: STATE_LAWS.filter(l => l.hasProtection).length,
          withoutProtection: STATE_LAWS.filter(l => !l.hasProtection).length,
        };
      }),
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert in the No Surprises Act (NSA), 45 CFR § 149.510, and state balance billing laws. Provide concise, accurate compliance guidance. Always cite specific regulatory provisions. Return JSON with fields: answer (string), sources (string array), confidence (0-1 number), suggestedActions (string array).",
            },
            {
              role: "user",
              content: `State: ${input.state}. ${disputeContext}\n\nWhat state balance billing laws apply to this dispute and what are the compliance requirements for NSA IDR? Include specific notice requirements, IDR timelines, and plan type considerations (ERISA vs state-regulated).`,
            },
          ],
          response_format: {
            type: "json_schema",
  ...
  expertReview: router({
    request: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const { disputes, disputeEvents, notifications } = await import(
          "../drizzle/schema"
        );
        ...
        await db.insert(disputeEvents).values({
          id: crypto.randomUUID(),
          disputeId: input.disputeId,
          step: "STEP_01_OPEN_NEGOTIATION_INITIATED" as const,
          eventType: "expert_review_requested",
          description: `Expert review requested: ${input.reason} (urgency: ${input.urgency})`,
          performedBy: ctx.user.id,
          createdAt: new Date(),
        });
        await db.insert(notifications).values({
          ...
          disputeId: input.disputeId,
          userId: ctx.user.id,
          notificationType: "expert_review",
          title: "Expert Review Requested",
          message: `Your expert review request has been received. Urgency: ${input.urgency}. Expected response: ${input.urgency === "critical" ? "4 hours" : input.urgency === "urgent" ? "24 hours" : "3 business days"}.`,
          isRead: false,
          createdAt: new Date(),
        });
        return {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a certified IDR expert specializing in the No Surprises Act. Analyze disputes and provide actionable expert guidance. Return JSON with fields: analysis (string), sources (string array), confidence (0-1 number), recommendations (string array).",
            },
            {
              role: "user",
              content: `Analyze this IDR dispute: ${disputeContext}\n\nProvide: (1) strength of the provider's position, (2) likelihood of success in IDR arbitration, (3) recommended negotiation strategy, (4) key regulatory arguments to raise under 45 CFR § 149.510, (5) comparable determination benchmarks and QPA methodology considerations.`,
            },
          ],
          response_format: {
            type: "json_schema",
    ...
    submit: publicProcedure
      .input(
        z.object({
          ...
          stakeholderRole: z
            .enum(["provider", "facility", "payer", "idr_entity", "other"])
            .optional(),
    ...
    summary: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        let query = db
          .select()
          .from(disputes)
          .where(eq(disputes.createdBy, ctx.user.id));
        ...
        const filtered =
          startMs > 0
            ? allDisputes.filter(d => (d.createdAt?.getTime() ?? 0) >= startMs)
            : allDisputes;
        const closed = filtered.filter(d => d.status === "closed");
        const won = closed.filter(
          (d: (typeof allDisputes)[0]) =>
            Number(d.determinationAmount ?? 0) >= Number(d.qpaAmount ?? 0)
        );
        const totalAmount = filtered.reduce(
          (s, d) => s + Number(d.billedAmount ?? 0),
          0
        );
        const avgDetermination = closed.length
          ? closed.reduce(
              (s: number, d: (typeof allDisputes)[0]) =>
                s + Number(d.determinationAmount ?? 0),
              0
            ) / closed.length
          : 0;
        const avgDaysToClose = closed.length
          ? closed.reduce((s: number, d: (typeof allDisputes)[0]) => {
              const ms =
                (d.updatedAt?.getTime() ?? Date.now()) -
                (d.createdAt?.getTime() ?? Date.now());
              return s + ms / 86400000;
            }, 0) / closed.length
          : 0;
        // byServiceType: count per type
        const byServiceType = Object.entries(
          filtered.reduce(
            (acc: Record<string, number>, d: (typeof allDisputes)[0]) => {
        ...
        const finMap: Record<
          string,
          { billed: number[]; qpa: number[]; det: number[] }
        > = {};
        ...
        const financialByServiceType = Object.entries(finMap).map(
          ([serviceType, vals]) => ({
            serviceType: serviceType.replace(/_/g, " "),
            avgBilled: vals.billed.length
              ? Math.round(
                  vals.billed.reduce((a, b) => a + b, 0) / vals.billed.length
                )
              : 0,
            avgQPA: vals.qpa.length
              ? Math.round(
                  vals.qpa.reduce((a, b) => a + b, 0) / vals.qpa.length
                )
              : 0,
            avgDetermination: vals.det.filter(v => v > 0).length
              ? Math.round(
                  vals.det.filter(v => v > 0).reduce((a, b) => a + b, 0) /
        ...
        const outcomeMap: Record<
          string,
          { month: string; won: number; lost: number; pending: number }
        > = {};
        for (const d of filtered) {
          const dt = d.createdAt ?? new Date();
          const key = `${MONTHS[dt.getMonth()]} ${dt.getFullYear()}`;
          ...
          if (!outcomeMap[key])
            outcomeMap[key] = { month: label, won: 0, lost: 0, pending: 0 };
          if (d.status === "closed") {
            if (Number(d.determinationAmount ?? 0) >= Number(d.qpaAmount ?? 0))
              outcomeMap[key].won++;
        ...
        const avgDaysByStep = IDR_STEPS.slice(0, 10).map(step => ({
          ...
          avgDays: stepDayMap[step]?.length
            ? Math.round(
                stepDayMap[step].reduce((a, b) => a + b, 0) /
                  stepDayMap[step].length
              )
            : 0,
        }));
        return {
          totalDisputes: filtered.length,
          totalAmount: Math.round(totalAmount),
          avgDetermination: Math.round(avgDetermination),
    ...
    exportCSV: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const allDisputes = await db
          ...
          .limit(10000);
        const filtered = startFilter
          ? allDisputes.filter(d => d.createdAt && d.createdAt >= startFilter)
          : allDisputes;
        // Build summary (reuse same logic as summary procedure)
        const closed = filtered.filter(d => d.status === "closed");
        const won = closed.filter(
          d => Number(d.determinationAmount ?? 0) >= Number(d.qpaAmount ?? 0)
        );
        const totalAmount = filtered.reduce(
          (s, d) => s + Number(d.billedAmount ?? 0),
          0
        ...
        const avgDetermination = closed.length
          ? closed.reduce((s, d) => s + Number(d.determinationAmount ?? 0), 0) /
            closed.length
          : 0;
        const avgDaysToClose = closed.length
          ? closed.reduce((s, d) => {
              const ms =
                (d.closedAt?.getTime() ?? Date.now()) -
                (d.createdAt?.getTime() ?? Date.now());
              return s + ms / 86400000;
            }, 0) / closed.length
          : 0;
  ...
  webhooks: router({
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const updated = await updateWebhook(ctx.user.id, id, {
          ...rest,
    ...
    delete: protectedProcedure
      .input(z.object({ id: z.string() }))
      .mutation(async ({ ctx, input }) => {
        const deleted = await deleteWebhook(ctx.user.id, input.id);
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          const payload = JSON.stringify({
            ...
            source: "HealthPoint IDR",
          });
          const res = await fetch(hook.url, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              "X-HealthPoint-Event": "test.ping",
            },
            body: payload,
            signal: AbortSignal.timeout(5000),
          });
          await updateWebhook(ctx.user.id, input.id, {
  ...
  predictions: router({
    get: protectedProcedure
      .input(z.object({ disputeId: z.string().min(1) }))
      .query(async ({ ctx, input }) => {
        return getGovernedOutcomePrediction({
          userId: ctx.user.id,
    ...
    generate: protectedProcedure
      .input(z.object({ disputeId: z.string().min(1) }))
      .mutation(async ({ ctx, input }) => {
        const context = await requireGovernedOutcomeContext({
          userId: ctx.user.id,
          userRole: ctx.user.role,
          disputeId: input.disputeId,
        });
        const prediction = await invokeGovernedGeorgetownRuntime(context);
        return persistGovernedOutcomePrediction(context, prediction);
      }),
  }),
  ...
  docIntelligence: router({
    analyze: protectedProcedure
      .input(
        z.object({
          ...
          disputeId: z.string().optional(),
          documentType: z
            .enum(["eob", "ra", "cms1500", "ub04", "appeal", "other"])
            .default("other"),
        })
      )
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          ...
          try {
            ...
            const result = await storagePut(
              `doc-analysis/${analysis.id}/${input.fileName}`,
              buffer,
              input.fileType
            );
            s3Key = result.key;
                ...
                schema: {
                  ...
                  properties: {
                    ...
                    patientDOB: {
                      type: "string",
                      description:
                        "Patient date of birth (YYYY-MM-DD or empty)",
                    },
                    patientId: {
                      type: "string",
                      description: "Patient ID or member ID",
                    },
                    providerName: {
                      type: "string",
                    ...
                    payerName: {
                      type: "string",
                      description: "Insurance company / payer name",
                    },
                    payerId: { type: "string", description: "Payer ID" },
                    claimNumber: {
                      type: "string",
                      description: "Claim number or reference",
                    },
                    dateOfService: {
                      type: "string",
                      description: "Date of service (YYYY-MM-DD or range)",
                    },
                    billedAmount: {
                      type: "string",
                    ...
                    allowedAmount: {
                      type: "string",
                      description: "Allowed/approved amount in dollars",
                    },
                    paidAmount: {
                      type: "string",
                    ...
                    denialReason: {
                      type: "string",
                      description: "Reason for denial or adjustment",
                    },
                    denialCode: {
                      type: "string",
                      description: "Denial or remark code (e.g., CO-45, PR-1)",
                    },
                    cptCodes: {
                      type: "array",
                      items: { type: "string" },
                    ...
                    serviceType: {
                      type: "string",
                      description:
                        "Type of service (e.g., Emergency, Radiology)",
                    },
                    facilityState: {
                      type: "string",
                      description:
                        "State where service was rendered (2-letter code)",
                    },
                    isOutOfNetwork: {
                      type: "boolean",
                      description: "Whether the provider is out-of-network",
                    },
                    nsaApplicable: {
                      type: "boolean",
                      description:
                        "Whether NSA/No Surprises Act likely applies",
                    },
                    rawText: {
                      type: "string",
                      description: "Full OCR text extracted from the document",
                    },
                    confidence: {
                      type: "integer",
    ...
    getDownloadUrl: protectedProcedure
      ...
      .query(async ({ input }) => {
        const analysis = await getDocumentAnalysis(input.id);
        if (!analysis?.s3Key)
          throw new TRPCError({ code: "NOT_FOUND", message: "No file stored" });
    ...
    progress: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const progress = getWorkflowProgress(
          dispute.currentStep as Parameters<typeof getWorkflowProgress>[0]
        );
        const validTransitions = getValidTransitions(
          dispute.currentStep as Parameters<typeof getValidTransitions>[0]
        );
        const stepDef =
          IDR_WORKFLOW_STEPS[
            dispute.currentStep as keyof typeof IDR_WORKFLOW_STEPS
          ];
      ...
      .query(async ({ ctx, input }) => {
        ...
        const conditions = input.stepId
          ? and(
              ...
              eq(stepNotes.stepId, input.stepId)
            )
          : eq(stepNotes.disputeId, input.disputeId);
  ...
  ledger: router({
    ...
    recordPayment: protectedProcedure
      .input(
        z.object({
          disputeId: z.string(),
          amountCents: z.string().regex(/^[1-9]\d{0,18}$/, "amountCents must be a canonical positive cents string"),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          amountCents = parseCentsString(input.amountCents);
        } catch (error) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: error instanceof Error ? error.message : "Payment amount must be canonical whole cents",
          });
        }
  ...
  // A provider adapter is not advertised until its mutually authenticated callback
  // and reconciliation evidence are operational. This route exposes only the
  // durable PostgreSQL lifecycle record under the owning dispute access boundary.
  mojaloop: router({
    transferStatus: protectedProcedure
      .input(z.object({ disputeId: z.string().min(1), transferId: z.string().uuid() }))
      .query(async ({ ctx, input }) => {
        await assertDisputeAccess(ctx.user.id, ctx.user.role, input.disputeId, "read");
        const transfer = await getSettlementTransfer(input.transferId);
        if (!transfer || transfer.disputeId !== input.disputeId) {
          throw new TRPCError({ code: "NOT_FOUND", message: "Settlement transfer not found" });
        }
        return {
    ...
    listByDispute: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        return (entries as Array<{ referenceId?: string | null }>).filter(
          e => e.referenceId?.startsWith("ML-") ?? false
        );
      }),
  }),
  ...
  temporal: router({
    readiness: adminProcedure.query(() => {
      try {
        ...
        return {
          configured: true,
          verification: config.usingDevelopmentDefaults
            ? ("unverified_default" as const)
        ...
        return {
          ...
          reason:
            error instanceof Error
              ? error.message
              : "Temporal configuration is invalid",
        };
      }
    }),
    checkConnection: adminProcedure.mutation(async ({ ctx }) => {
      try {
        ...
        const verification = config.usingDevelopmentDefaults
          ? ("unverified_default" as const)
          : ("operator_configured" as const);
        const evidence = {
        ...
        return {
          reachable: true,
          message: config.usingDevelopmentDefaults
            ? "Temporal accepted a strictly authenticated connection using unverified defaults."
            : "Temporal accepted a strictly authenticated connection.",
          verification,
          namespace: config.namespace,
          taskQueue: config.taskQueue,
          auditId: audit.id,
        ...
        const recovery = describeTemporalFailure(
          error,
          (error as { recovery?: { attempts?: number } })?.recovery?.attempts ??
            1
        );
        const evidence = {
          kind: "TEMPORAL_SUPERVISED_READ_ONLY_STATUS_CHECK",
    ...
    connectionAlerts: adminProcedure.query(async () => {
      ...
      try {
        const details = lastFailure?.newValue
          ? (JSON.parse(lastFailure.newValue) as {
              recovery?: TemporalRecoveryDetails;
            })
          : null;
        ...
        return rows.map(row => {
          ...
          try {
            details = row.newValue
              ? (JSON.parse(row.newValue) as Record<string, unknown>)
              : null;
          } catch {
            details = null;
          }
      ...
      .query(async ({ ctx, input }) => {
        ...
        try {
          ...
          throw new TRPCError({
            code: "PRECONDITION_FAILED",
            message:
              "Temporal workflow status is unavailable; strict TLS connectivity and a deployed workflow execution are required",
            cause: error,
          });
        }
      }),
      ...
      .query(async ({ ctx, input }) => {
        ...
        try {
          ...
          throw new TRPCError({
            code: "PRECONDITION_FAILED",
            message:
              "Temporal workflow listing is unavailable; strict TLS connectivity is required",
            cause: error,
          });
        }
      }),
    startDisputeWorkflow: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if (
          process.env.PAYMENT_EXECUTION_MODE &&
          process.env.PAYMENT_EXECUTION_MODE !== "disabled"
        ) {
          throw new TRPCError({
            code: "PRECONDITION_FAILED",
            message:
              "Temporal dispatch is restricted to the current non-payment execution mode",
          });
        }
        const dispute = await getDisputeById(input.disputeId);
        if (!dispute)
        ...
        try {
          const result = await startDisputeTemporalWorkflow(
            input.disputeId,
            ctx.user.id
          );
          ...
          throw new TRPCError({
            ...
            message:
              error instanceof Error
                ? error.message
                : "Temporal workflow dispatch is unavailable",
            cause: error,
          });
        }
      }),
  ...
  lakehouse: router({
    export: protectedProcedure
      .input(
        z.object({
          tables: z.array(
            z.enum(["disputes", "documents", "audit", "ledger", "events"])
          ),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const { url } = await storagePut(
          ...
          input.format === "ndjson" ? "application/x-ndjson" : "text/csv"
        );
    ...
    list: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        return db
          ...
          .where(
            and(
              eq(disputeComments.disputeId, input.disputeId),
              isNull(disputeComments.parentId)
            )
          )
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const [existing] = await db
          .select()
          .from(disputeComments)
          .where(eq(disputeComments.id, input.id))
          .limit(1);
        if (!existing) throw new TRPCError({ code: "NOT_FOUND" });
        if (existing.authorId !== ctx.user.id && ctx.user.role !== "admin")
          throw new TRPCError({ code: "FORBIDDEN" });
        const [updated] = await db
          .update(disputeComments)
          .set({ content: input.content, edited: true, updatedAt: new Date() })
          .where(eq(disputeComments.id, input.id))
          .returning();
        return updated;
      }),
    ...
    replies: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        return db
          .select()
          .from(disputeComments)
          .where(eq(disputeComments.parentId, input.parentId))
          .orderBy(disputeComments.createdAt);
      }),
    summarize: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const commentText = allComments
          .map(
            (c, i) =>
              `[${i + 1}] ${c.authorName} (${new Date(c.createdAt!).toLocaleDateString()}): ${c.content}`
          )
          .join("\n");
        const response = await invokeLLM({
          messages: [
            {
              role: "system",
              content:
                "You are an expert IDR dispute analyst. Summarize the following dispute discussion comments concisely. Extract: (1) key points raised, (2) any agreements or disagreements, (3) action items or next steps, (4) overall sentiment. Be factual and neutral. Use 3-5 bullet points maximum.",
            },
            {
              role: "user",
  ...
  payerContacts: router({
      ...
      .query(async ({ input }) => {
        ...
        if (input.search && input.search.trim().length > 1) {
          ...
          const all = await db
            .select()
            .from(payerContacts)
            .orderBy(payerContacts.payerName);
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const { id, ...rest } = input;
        const [updated] = await db
          .update(payerContacts)
          .set({ ...rest, updatedAt: new Date() })
          .where(eq(payerContacts.id, id))
          .returning();
        // Sync to OpenSearch
        if (updated)
          indexDocument(
  ...
  sla: router({
    breaches: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        if (input.disputeId)
          conditions.push(eq(slaBreaches.disputeId, input.disputeId));
        if (input.severity)
          conditions.push(eq(slaBreaches.severity, input.severity));
        const query = db
          .select()
          .from(slaBreaches)
          .orderBy(slaBreaches.detectedAt);
        return conditions.length > 0
          ? query.where(and(...conditions)).limit(input.limit)
    ...
    check: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const stepDeadlines: Record<string, number> = {
          STEP_01_OPEN_NEGOTIATION: 30,
          STEP_02_IDR_NOTICE: 4,
          STEP_03_IDR_INITIATION: 3,
          STEP_04_ENTITY_SELECTION: 3,
          STEP_05_ENTITY_SELECTION_PERIOD: 3,
          STEP_06_ENTITY_CONFIRMATION: 1,
          STEP_07_ADDITIONAL_INFO: 10,
          STEP_08_PRELIMINARY_PAYMENT: 30,
          STEP_09_OFFER_SUBMISSION: 10,
          STEP_10_ARBITRATION: 30,
          STEP_11_DETERMINATION: 30,
          STEP_12_PAYMENT: 30,
        };
      ...
      .query(async ({ input }) => {
        ...
        return active.map(d => {
          ...
          const deadlineDate: Date | null =
            (step === "STEP_01_OPEN_NEGOTIATION_INITIATED" ||
              step === "STEP_02_OPEN_NEGOTIATION_PERIOD") &&
            d.openNegotiationDeadline
          ...
          const percent =
            totalMs > 0
              ? Math.min(Math.round((elapsedMs / totalMs) * 100), 110)
              : 0;
    ...
    changeStatus: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        await db
          ...
          .where(inArray(disputesTable.id, input.ids));
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        for (let i = 1; i < lines.length; i++) {
          try {
            ...
            if (!row.respondingPartyName && !row.payer) {
              skipped++;
              continue;
            }
            await createDispute({
  ...
  webhookReplay: router({
    list: protectedProcedure
      .input(
        z.object({
          status: z.enum(["failed", "pending", "delivered"]).optional(),
          limit: z.number().min(1).max(200).default(50),
        })
      )
      .query(async ({ input }) => {
        ...
        if (input.status) {
          const results = await db
            .select()
            .from(webhookDeliveries)
            .where(eq(webhookDeliveries.status, input.status))
            .orderBy(desc(webhookDeliveries.createdAt))
            .limit(input.limit);
          return results;
        }
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (existing.length > 0) {
          ...
          await db
            ...
            .values({ id: nanoid(), userId: ctx.user.id, ...input });
        }
        return { success: true };
      }),
  }),
  ...
  providerAcceptance: router({
    list: adminProcedure.query(async () => {
      ...
      return db
        .select()
        .from(providerSandboxAcceptances)
        .orderBy(desc(providerSandboxAcceptances.submittedAt));
    }),
        ...
        z.object({
          ...
          sandboxBaseUrl: z
            ...
            .refine(
              ...
              "Sandbox URL must use HTTPS"
            )
            .optional(),
          providerReference: z.string().trim().max(160).optional(),
          mtlsEvidenceState: z.enum([
            "pending",
            "submitted",
            "verified_by_provider",
            "rejected",
          ]),
          reconciliationEvidenceState: z.enum([
            "pending",
            "submitted",
            "verified_by_provider",
            "rejected",
      ...
      .mutation(async ({ ctx, input }) => {
        if (
          ...
          throw new TRPCError({
            code: "PRECONDITION_FAILED",
            message:
              "Provider evidence recording is permitted only while payment execution remains disabled",
          });
        }
        const db = await getDb();
        if (!db)
        ...
        const { nanoid } = await import("nanoid");
        const { computeAcceptanceStatus, validateProviderEvidenceInput } =
          await import("./provider-acceptance");
        try {
          ...
          throw new TRPCError({
            ...
            message:
              error instanceof Error
                ? error.message
                : "Invalid provider acceptance evidence",
          });
        }
        const status = computeAcceptanceStatus(
          input.mtlsEvidenceState,
          input.reconciliationEvidenceState,
          input.bilateralAttestationReference
        );
        const [record] = await db
          .insert(providerSandboxAcceptances)
          .values({
            id: nanoid(),
            submittedBy: ctx.user.id,
            status,
  ...
  watchlist: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      ...
      const entries = await db
        .select()
        .from(disputeWatchlist)
        .where(eq(disputeWatchlist.userId, ctx.user.id))
        .orderBy(disputeWatchlist.createdAt);
      if (entries.length === 0) return [];
      const disputeIds = entries.map(e => e.disputeId);
      const { inArray } = await import("drizzle-orm");
      const relatedDisputes = await db
        .select({
          id: disputesTable.id,
          referenceNumber: disputesTable.referenceNumber,
          status: disputesTable.status,
          respondingPartyName: disputesTable.respondingPartyName,
          billedAmount: disputesTable.billedAmount,
        })
        .from(disputesTable)
        .where(inArray(disputesTable.id, disputeIds));
      const disputeMap = Object.fromEntries(
        relatedDisputes.map(d => [d.id, d])
      );
      return entries.map(e => ({
        ...e,
        dispute: disputeMap[e.disputeId] ?? null,
      }));
    }),
    add: protectedProcedure
      .input(
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const existing = await db
          ...
          .where(
            and(
              eq(disputeWatchlist.userId, ctx.user.id),
              eq(disputeWatchlist.disputeId, input.disputeId)
            )
          )
          .limit(1);
        if (existing.length > 0)
          throw new TRPCError({
            code: "CONFLICT",
            message: "Already watching this dispute",
          });
        const [entry] = await db
          .insert(disputeWatchlist)
  ...
  escalations: router({
    list: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const conditions = [];
        if (input.disputeId)
          conditions.push(eqOp(disputeEscalations.disputeId, input.disputeId));
        if (input.status)
          conditions.push(eqOp(disputeEscalations.status, input.status as any));
        if (ctx.user.role !== "admin")
          conditions.push(eqOp(disputeEscalations.raisedBy, ctx.user.id));
        return db
          .select()
          .from(disputeEscalations)
          .where(conditions.length ? andOp(...(conditions as any)) : undefined)
          .orderBy(disputeEscalations.createdAt);
      }),
    create: protectedProcedure
      .input(
        z.object({
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        const [esc] = await db
          .insert(disputeEscalations)
          .values({
            disputeId: input.disputeId,
            raisedBy: ctx.user.id,
            raisedByName: ctx.user.name ?? "Unknown",
    ...
    resolve: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const [updated] = await db
          ...
          .where(eq(disputeEscalations.id, input.id))
          .returning();
        return updated;
      }),
  }),
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (input.disputeId)
          return db
            .select()
            .from(disputeAppeals)
            .where(eq(disputeAppeals.disputeId, input.disputeId))
            .orderBy(disputeAppeals.createdAt);
        if (ctx.user.role === "admin")
          return db
            .select()
            .from(disputeAppeals)
        ...
        return db
          .select()
          .from(disputeAppeals)
          .where(eq(disputeAppeals.submittedBy, ctx.user.id))
          .orderBy(disputeAppeals.createdAt);
      }),
    create: protectedProcedure
      .input(
        z.object({
          disputeId: z.string(),
          groundsForAppeal: z.string().min(20).max(5000),
          supportingEvidence: z.string().optional(),
          originalDetermination: z.string().optional(),
        })
      )
      .mutation(async ({ ctx, input }) => {
        const db = await getDb();
    ...
    decide: protectedProcedure
      .input(
        z.object({
          ...
          appealDecision: z.string().min(10),
        })
      )
      .mutation(async ({ ctx, input }) => {
        if (ctx.user.role !== "admin")
        ...
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        const [updated] = await db
          .update(disputeAppeals)
          .set({
            status: input.decision,
            appealDecision: input.appealDecision,
            decidedAt: new Date(),
            updatedAt: new Date(),
          })
          .where(eq(disputeAppeals.id, input.id))
          .returning();
        return updated;
      }),
  }),
  ...
  narratives: router({
    list: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        return db
          .select()
          .from(disputeNarratives)
          .where(eq(disputeNarratives.disputeId, input.disputeId))
          .orderBy(disputeNarratives.createdAt);
      }),
    generate: protectedProcedure
      .input(
        z.object({
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert healthcare attorney specializing in NSA/IDR disputes. Write professional, factual legal documents for IDR proceedings. Use formal legal language appropriate for submission to a certified IDR entity. Do not fabricate specific dollar amounts or dates not provided.",
            },
            {
              role: "user",
              content: `Write a ${typeLabels[input.narrativeType]} for the following IDR dispute:\n\nReference: ${dispute.referenceNumber}\nInitiating Party: ${dispute.initiatingPartyName}\nResponding Party: ${dispute.respondingPartyName}\nService Type: ${dispute.serviceType}\nBilled Amount: $${dispute.billedAmount}\nPatient State: ${dispute.patientState}\n${input.context ? `\nAdditional context: ${input.context}` : ""}\n\nWrite a professional ${typeLabels[input.narrativeType]} of approximately 400-600 words.`,
            },
          ],
        });
        const rawContent = response?.choices?.[0]?.message?.content;
    ...
    approve: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        const [updated] = await db
          .update(disputeNarratives)
          .set({
            approved: true,
            approvedBy: ctx.user.id,
            approvedAt: new Date(),
          })
          .where(eq(disputeNarratives.id, input.id))
          .returning();
        return updated;
      }),
    delete: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        await db
          .delete(disputeNarratives)
          .where(
            and(
              eq(disputeNarratives.id, input.id),
              eq(disputeNarratives.generatedBy, ctx.user.id)
            )
          );
        return { success: true };
      }),
  ...
  docExpiry: router({
    list: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        if (input.disputeId)
          conditions.push(
            eqOp(documentExpiryAlerts.disputeId, input.disputeId)
          );
        if (!input.showDismissed)
          conditions.push(eqOp(documentExpiryAlerts.dismissed, false));
        return db
          .select()
          .from(documentExpiryAlerts)
          .where(conditions.length ? andOp(...conditions) : undefined)
          .orderBy(documentExpiryAlerts.expiresAt);
      }),
    add: protectedProcedure
      .input(
        z.object({
    ...
    dismiss: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        await db
          .update(documentExpiryAlerts)
          .set({ dismissed: true })
          .where(eq(documentExpiryAlerts.id, input.id));
        return { success: true };
      }),
  }),
  ...
  fhirCapability: router({
    fetch: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const id = nanoid();
        const [cap] = await db
          .insert(fhirCapabilityStatements)
          .values({
            id,
    ...
    list: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        return db
          .select()
          .from(fhirCapabilityStatements)
          .where(
            eq(fhirCapabilityStatements.emrConnectionId, input.emrConnectionId)
          );
      }),
  }),
    ...
    listTokens: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        return db
          ...
          .where(
            and(
              eq(smartTokens.emrConnectionId, input.emrConnectionId),
              eq(smartTokens.userId, ctx.user.id)
  ...
  bulkFhir: router({
    startExport: protectedProcedure
      .input(
        z.object({
          ...
          resourceTypes: z
            .array(z.string())
            .default(["Patient", "Claim", "Coverage", "ExplanationOfBenefit"]),
    ...
    cancelJob: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        await db
          .update(bulkFhirExportJobs)
          .set({ status: "cancelled" })
          .where(eq(bulkFhirExportJobs.id, input.jobId));
        return { success: true };
      }),
  }),
  ...
  cdsHooksRouter: router({
    list: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        return db
          .select()
          .from(cdsHooks)
          .where(eq(cdsHooks.emrConnectionId, input.emrConnectionId));
      }),
    ...
    toggleStatus: protectedProcedure
      .input(
        z.object({ id: z.string(), status: z.enum(["active", "inactive"]) })
      )
      .mutation(async ({ input }) => {
        const db = await getDb();
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
  ...
  daVinci: router({
    list: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        const conditions = [];
        if (input.disputeId)
          conditions.push(eq(daVinciTransactions.disputeId, input.disputeId));
        return db
          .select()
          .from(daVinciTransactions)
          .where(conditions.length ? conditions[0] : undefined)
          .orderBy(daVinciTransactions.createdAt) as Promise<
          (typeof daVinciTransactions.$inferSelect)[]
        >;
      }),
    ...
    getCompleteness: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        const [result] = await db
          .select()
          .from(uscdiDataElements)
          .where(eq(uscdiDataElements.disputeId, input.disputeId))
          .limit(1);
        return result ?? null;
      }),
    updateCompleteness: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (existing.length > 0) {
          ...
          await db.insert(uscdiDataElements).values({
            id: nanoid() as string,
            disputeId: input.disputeId,
            completenessScore: score,
            missingElements: missing,
  ...
  fhirCache: router({
    list: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        const conditions = [];
        if (input.disputeId)
          conditions.push(eq(fhirResourceCache.disputeId, input.disputeId));
        if (input.emrConnectionId)
          conditions.push(
            eq(fhirResourceCache.emrConnectionId, input.emrConnectionId)
          );
        if (input.resourceType)
          conditions.push(
            eq(fhirResourceCache.resourceType, input.resourceType)
          );
        return db
          .select()
          .from(fhirResourceCache)
          .where(conditions.length ? conditions[0] : undefined)
          .orderBy(fhirResourceCache.fetchedAt) as Promise<
          (typeof fhirResourceCache.$inferSelect)[]
        >;
      }),
    purge: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
        await db
          .delete(fhirResourceCache)
          .where(eq(fhirResourceCache.emrConnectionId, input.emrConnectionId));
        return { success: true };
      }),
  }),
  ...
  smartForm: router({
    ...
    extract: protectedProcedure
      .input(
        z.object({
          inputType: z
            .enum(["text", "pdf_base64", "fhir_json", "url"])
            .default("text"),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        > = {
          dispute: {
            ...
            providerNPI: {
              type: "string",
              description: "Provider NPI (10-digit number)",
            },
            payerName: {
              type: "string",
              description: "Insurance company / payer name",
            ...
            allowedAmount: {
              type: "number",
              description: "Payer allowed amount in USD",
            },
            qpaAmount: {
              type: "number",
              description: "Qualifying Payment Amount (QPA) in USD",
            },
            cptCodes: {
              type: "string",
              description: "CPT/procedure codes (comma-separated)",
            },
            diagnosisCodes: {
              type: "string",
            ...
            placeOfService: {
              type: "string",
              description: "Place of service code or description",
            },
            serviceType: {
              type: "string",
              description:
                "Type of service (emergency, non-emergency, ancillary, etc.)",
            },
          },
          offer: {
            offerAmount: {
              type: "number",
              description: "Proposed settlement amount in USD",
            },
            rationale: {
              type: "string",
              description: "Justification or reasoning for the offer",
            },
            counterOfferDeadline: {
              type: "string",
              description: "Deadline for counter-offer response (YYYY-MM-DD)",
            },
            supportingBenchmark: {
              type: "string",
              description:
                "Benchmark or reference used (e.g., Medicare rate, QPA)",
            },
          },
          cms_submission: {
            submissionType: {
              type: "string",
              description:
                "Type of CMS submission (IDR initiation, appeal, etc.)",
            },
            referenceNumber: {
              type: "string",
              description: "CMS reference or tracking number",
            },
            submissionDate: {
              type: "string",
              description: "Date of submission (YYYY-MM-DD)",
            },
            determinationDeadline: {
              type: "string",
              description: "Expected determination deadline (YYYY-MM-DD)",
            },
          },
          emr_onboarding: {
            ehrVendor: {
              type: "string",
              description: "EHR vendor name (Epic, Cerner, Meditech, etc.)",
            },
          ...
          generic: {
            ...
            date: {
              type: "string",
              description: "Primary date in the document (YYYY-MM-DD)",
            },
            ...
            referenceNumber: {
              type: "string",
              description: "Any reference, claim, or tracking number",
            },
            summary: {
              type: "string",
        ...
        const schemaDescription = Object.entries(schema)
          .map(
            ([k, v]) =>
              `  "${k}": { "value": <${v.type} or null>, "confidence": <0-100>, "source": "<brief quote or reason>" }`
          )
          .join(",\n");

        const contentPreview =
          input.inputType === "pdf_base64"
...
IMPORTANT: Return ONLY the JSON object, no markdown, no explanation.`;
        ...
        try {
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a precise medical billing data extraction assistant. Always return valid JSON only.",
              },
                ...
                extractedFields[key] = {
                  ...
                  confidence:
                    typeof v.confidence === "number"
                      ...
                      : 50,
                  source:
                    typeof v.source === "string" ? v.source : "LLM extraction",
                };
              }
            }
          } catch {
          ...
          const highConfidenceCount = Object.values(extractedFields).filter(
            f => f.confidence >= 80
          ).length;
          const lowConfidenceCount = Object.values(extractedFields).filter(
            f => f.confidence < 50
          ).length;
          const overallConfidence =
            fieldCount > 0
              ? Math.round(
                  Object.values(extractedFields).reduce(
  ...
  orgSettings: router({
    get: protectedProcedure.query(async ({ ctx }) => {
      ...
      const rows = await db
        .select()
        .from(orgSettings)
        .where(eq(orgSettings.userId, ctx.user.id))
        .limit(1);
      return rows[0] ?? null;
    }),
    upsert: protectedProcedure
  ...
  totp: router({
    ...
    generateSecret: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const { generateSecret: genSecret, generateURI } = await import(
          "otplib"
        );
    ...
    verify: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const rows = await db
          .select({ secret: totpSecrets.secret })
          .from(totpSecrets)
          .where(
            and(
              eq(totpSecrets.userId, ctx.user.id),
              eq(totpSecrets.status, "pending")
            )
          )
          .limit(1);
        if (!rows.length)
    ...
    disable: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!rows.length)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "2FA is not currently active",
          });
        const { verify: totpVerify } = await import("otplib");
        const verifyResult = await totpVerify({
          token: input.code,
    ...
    getBackupCodes: protectedProcedure.query(async ({ ctx }) => {
      ...
      const rows = await db
        .select({
          backupCodes: totpSecrets.backupCodes,
          usedBackupCodes: totpSecrets.usedBackupCodes,
        })
        .from(totpSecrets)
        .where(eq(totpSecrets.userId, ctx.user.id))
        .limit(1);
      ...
      return {
        ...
        usedBackupCodes: JSON.parse(
          rows[0].usedBackupCodes ?? "[]"
        ) as string[],
      };
    }),
  }),
  ...
  qpaBenchmarks: router({
    list: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        const filtered = input.search
          ? rows.filter(
              r =>
                r.cptCode.includes(input.search!) ||
                r.description
                  .toLowerCase()
    ...
    stateModifiers: publicProcedure.query(async () => {
      ...
      return Object.fromEntries(
        rows.map(r => [r.stateCode, parseFloat(r.modifier)])
      );
    }),
  }),

  // ─── Regulatory Change Feed ─────────────────────────────────────────────────
  // Curated records may be listed only after an authorized source-ingestion process
  // has recorded them. No product route seeds legal or CMS guidance.
    ...
    list: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        if (input.search && input.search.trim().length > 1) {
          const results = await search({
            q: input.search,
            entityTypes: ["regulatory"],
            limit: input.limit,
          });
          const ids = new Set(results.hits.map(hit => hit.id));
          const rows = await db
            .select()
            ...
            .limit(500);
          let filtered = rows.filter(row => ids.has(row.id));
          if (input.category && input.category !== "all") {
            filtered = filtered.filter(row => row.category === input.category);
          }
          if (input.impact && input.impact !== "all") {
            filtered = filtered.filter(row => row.impactLevel === input.impact);
          }
  ...
  expertPanelDB: router({
    list: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        let filtered = rows;
        if (input.specialty && input.specialty !== "all")
          filtered = filtered.filter(r => r.specialty === input.specialty);
        if (input.availability && input.availability !== "all")
          filtered = filtered.filter(
            r => r.availability === input.availability
          );
        return filtered;
      }),
      ...
      const experts = [
        {
          ...
          bio: "Board-certified emergency physician with dual MD/JD degree specializing in NSA surprise billing disputes and QPA methodology challenges.",
        },
        {
          id: "exp-002",
          name: "Dr. Michael Torres",
          credentials: "MD, MBA",
          specialty: "anesthesiology" as const,
          yearsExperience: 22,
          casesHandled: 289,
          successRate: "91%",
          avgResponseHours: 8,
          availability: "available" as const,
          bio: "Anesthesiologist with extensive experience in facility-based billing disputes and CRNA supervision billing challenges.",
        },
        {
          id: "exp-003",
          name: "Jennifer Walsh",
          credentials: "JD, CHFP",
          specialty: "air_ambulance" as const,
          yearsExperience: 12,
          casesHandled: 156,
          successRate: "88%",
          avgResponseHours: 12,
          availability: "busy" as const,
          bio: "Healthcare attorney specializing in air ambulance billing disputes, CAMTS accreditation, and federal preemption issues.",
        },
        {
          id: "exp-004",
          name: "Dr. Robert Kim",
          ...
          avgResponseHours: 6,
          availability: "available" as const,
          bio: "Interventional radiologist with expertise in teleradiology billing, global vs. technical component disputes, and RVU-based QPA challenges.",
        },
        {
          id: "exp-005",
          name: "Dr. Amanda Foster",
          credentials: "MD, FCAP",
          specialty: "pathology" as const,
          yearsExperience: 20,
          casesHandled: 178,
          successRate: "89%",
          avgResponseHours: 24,
          availability: "available" as const,
          bio: "Pathologist specializing in clinical laboratory billing disputes, specimen handling fees, and molecular diagnostics reimbursement.",
        },
        {
          id: "exp-006",
          name: "Dr. James Okafor",
          credentials: "MD, FACP",
          specialty: "hospitalist" as const,
          yearsExperience: 14,
          casesHandled: 267,
          successRate: "87%",
          avgResponseHours: 8,
          availability: "available" as const,
          bio: "Hospitalist with deep expertise in observation vs. inpatient status disputes, discharge planning billing, and SNF transition claims.",
        },
      ];
      for (const e of experts) {
        const existing = await db
          .select({ id: expertPanel.id })
          .from(expertPanel)
          .where(eq(expertPanel.id, e.id))
          .limit(1);
        if (!existing.length) {
        ...
        const rows = await db
          ...
          .where(
            and(
              ...
              input.disputeId
                ? eq(complianceChecks.disputeId, input.disputeId)
                : sql`1=1`
            )
          )
          .orderBy(
            asc(complianceChecks.sectionKey),
            asc(complianceChecks.itemKey)
          );
        return rows;
      }),
    upsert: protectedProcedure
    ...
    seed: protectedProcedure.mutation(async ({ ctx }) => {
      ...
      const entries = [
        {
          ...
          description:
            "Added persistent left sidebar navigation to all authenticated pages, providing consistent access to all features without page-level headers.",
        ...
        },
        {
          ...
          description:
            "Implemented real-time session expiry countdown modal with 'Stay Signed In' refresh capability and 5-minute advance warning.",
          category: "feature" as const,
          isHighlight: true,
        },
        {
          ...
          title: "Step Advancement Confirmation Dialog",
          description:
            "Added confirmation dialog to all dispute step advancement CTAs to prevent accidental workflow progression.",
        ...
        },
        {
          ...
          version: "2.3.0",
          releasedAt: new Date("2025-07-10"),
          title: "Workflow Progress Bar Fix",
          description:
            "Fixed progress bar showing 0% when a dispute was at Step 1. Progress now correctly includes the current active step.",
        ...
        },
        {
          ...
          title: "IDR Entity Dashboard KPI Skeleton Loaders",
          description:
            "Added animated skeleton loaders to KPI cards on the IDR Entity Dashboard to eliminate flash-of-zeros on initial load.",
          category: "improvement" as const,
          isHighlight: false,
        },
        {
          ...
          version: "2.2.0",
          releasedAt: new Date("2025-07-01"),
          title: "Database-Backed Settings",
          description:
            "GlobalSettings, TwoFactorAuth, QPA Benchmarks, Regulatory Feed, Expert Panel, and Compliance Checklist are now fully persisted to PostgreSQL.",
          category: "feature" as const,
          isHighlight: true,
        },
        {
          ...
          title: "Keycloak Forward Authentication",
          description:
            "Introduced Keycloak OIDC forward authentication, supporting PKCE, token refresh, and multi-realm configuration.",
        ...
        },
        {
          ...
          title: "19-Step IDR Workflow Engine",
          description:
            "Complete implementation of the NSA Independent Dispute Resolution 19-step workflow with statutory deadlines, SLA monitoring, and automated notifications.",
          category: "feature" as const,
          isHighlight: true,
        },
      ];
      for (const e of entries) {
        const existing = await db
          ...
          .from(changelogEntries)
          .where(eq(changelogEntries.id, e.id))
          .limit(1);
        if (!existing.length) {
    ...
    request: protectedProcedure
      .input(
        z.object({
          ...
          amountCents: z.string().regex(/^[1-9]\d{0,18}$/, "amountCents must be a canonical positive cents string").transform(value => parseCentsString(value)),
    ...
    markSubmitted: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!transfer)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Settlement transfer not found",
          });
        return markSettlementTransferSubmitted({
          ...input,
          actorId: ctx.user.id,
        });
      }),
  ...
  tigerbeetleFinality: router({
    listByDispute: protectedProcedure
      .input(z.object({ disputeId: z.string().uuid() }))
      .query(async ({ ctx, input }) => {
        await assertDisputeAccess(ctx.user.id, ctx.user.role, input.disputeId, "read");
        return listTigerBeetleFinalityIntentsForDispute(input.disputeId);
      }),
    listMappingDrafts: adminProcedure.query(() => listTigerBeetleFinalityMappingDrafts()),
    mappingActivationReadiness: adminProcedure
      .input(z.object({ mappingId: z.string().uuid() }))
      .query(({ input }) => getTigerBeetleFinalityMappingActivationReadiness(input.mappingId)),
    activateMapping: adminProcedure
      .input(z.object({ mappingId: z.string().uuid(), evidenceBundleId: z.string().min(1).max(64) }))
      .mutation(({ ctx, input }) => activateTigerBeetleFinalityMapping({ ...input, verifiedBy: ctx.user.id })),
    listSubmissionAuthorizations: adminProcedure
      .input(z.object({ intentId: z.string().uuid() }))
      .query(({ input }) => listTigerBeetleFinalitySubmissionAuthorizations(input.intentId)),
    requestSubmissionAuthorization: adminProcedure
      .input(z.object({
        intentId: z.string().uuid(),
        changeTicket: z.string().regex(/^CHG-[A-Za-z0-9._-]{3,120}$/),
        requestReason: z.string().trim().min(12).max(1000),
      }))
      .mutation(({ ctx, input }) => requestTigerBeetleFinalitySubmissionAuthorization({ ...input, requestedBy: ctx.user.id })),
    approveSubmissionAuthorization: adminProcedure
      .input(z.object({ authorizationId: z.string().uuid(), expiresInMinutes: z.number().int().min(5).max(30) }))
      .mutation(({ ctx, input }) => approveTigerBeetleFinalitySubmissionAuthorization({ ...input, approvedBy: ctx.user.id })),
    cancelSubmissionAuthorization: adminProcedure
      .input(z.object({ authorizationId: z.string().uuid() }))
      .mutation(({ ctx, input }) => cancelTigerBeetleFinalitySubmissionAuthorization({ ...input, cancelledBy: ctx.user.id })),
    executeSubmissionAuthorization: adminProcedure
      .input(z.object({ authorizationId: z.string().uuid() }))
      .mutation(({ ctx, input }) => executeTigerBeetleFinalitySubmissionAuthorization({ ...input, executedBy: ctx.user.id })),
    createMappingDraft: adminProcedure
      .input(z.object({
        provider: z.string().trim().min(2).max(64),
        currency: z.literal("USD"),
        debitAccountId: z.string().regex(/^[1-9]\d{0,38}$/),
        creditAccountId: z.string().regex(/^[1-9]\d{0,38}$/),
        ledger: z.number().int().min(1).max(0xffff_ffff),
        code: z.number().int().min(1).max(0xffff),
        mappingVersion: z.number().int().min(1),
        approvalReference: z.string().trim().min(8).max(128).regex(/^[A-Za-z0-9._:-]+$/),
      }))
      .mutation(async ({ ctx, input }) => createTigerBeetleFinalityMappingDraft(input, ctx.user.id)),
  }),
  ...
  settlementProofs: router({
    ...
    openExceptions: adminProcedure.query(async () =>
      listSettlementExceptionReviews(true)
    ),
    ...
    configureDailySchedule: adminProcedure
      .input(
        z.object({
          cron: z.string().trim().min(11).max(64).default("0 0 2 * * *"),
        })
      )
      .mutation(async ({ input }) =>
        configureDailyBalanceProofSchedule({ cron: input.cron })
      ),
  }),

  heartbeatOperations: router({
    list: adminProcedure.query(async () => listSchedulerJobs()),
  }),
  modelGovernance: router({
          ...
          predictions: z
            .array(
              z.object({
                ...
                ensemble: z.array(z.number().min(0).max(1)).min(3).max(101),
            ...
            .min(100),
          thresholds: z.object({
            maxBrierScore: z.number().min(0).max(1),
            maxExpectedCalibrationError: z.number().min(0).max(1),
            minAuc: z.number().min(0).max(1),
            maxSubgroupCalibrationGap: z.number().min(0).max(1),
            minCoverage: z.number().min(0).max(1),
            maxIntervalWidth: z.number().min(0).max(1),
          }),
        })
      )
      .mutation(async ({ input }) => {
        const { validateGeorgetownProbability } = await import(
          "./services/model-governance"
        );
        return validateGeorgetownProbability(input);
      }),
  }),
});
